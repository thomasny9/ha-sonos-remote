class SonosRemoteCard extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.querySelector("#openpresets")?.addEventListener("click",()=>{this._settingsOpen=false;this._view="presets";this._render();});
    this.shadowRoot.querySelector("#presetback")?.addEventListener("click",()=>{this._view="now";this._render();});
    this.shadowRoot.querySelectorAll("[data-preset-toggle]").forEach(el=>el.addEventListener("click",async()=>{const id=el.dataset.presetToggle,c=this._roomSetting(id),enabled=!c.start_volume_enabled;await this._saveRoomSetting(id,enabled,c.start_volume);el.classList.toggle("on",enabled);const slider=this.shadowRoot.querySelector(`[data-preset-vol="${id}"]`);if(slider)slider.disabled=!enabled;}));
    this.shadowRoot.querySelectorAll("[data-preset-vol]").forEach(el=>{el.addEventListener("input",e=>{const n=this.shadowRoot.querySelector(`[data-preset-num="${e.target.dataset.presetVol}"]`);if(n)n.textContent=e.target.value+"%";});el.addEventListener("change",async e=>{const id=e.target.dataset.presetVol,c=this._roomSetting(id);await this._saveRoomSetting(id,c.start_volume_enabled,Number(e.target.value));});});
