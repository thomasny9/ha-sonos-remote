class SonosRemoteCard extends HTMLElement {
  ...
  _recentHtml() {
    ...
    return `<div class="sectiontitle">Recently Played</div>`+items.map(item=>{const uri=item.uri||item.media_content_id||"";const type=item.media_type||"track";const name=item.name||item.title||"Unknown";const sub=item.artist||item.album||String(type).replaceAll("_"," ");return `<button class="fav maitem" data-recent-uri="${this._esc(uri)}" data-recent-type="${this._esc(type)}"><span class="favart"><ha-icon icon="mdi:history"></ha-icon></span><span><span class="favname">${this._esc(name)}</span><span class="favsub">${this._esc(sub||"Music Assistant")}</span></span><ha-icon icon="mdi:play"></ha-icon></button>`}).join("");
  }
