class SonosRemoteCard extends HTMLElement {
  ...
  _render() {
    ...
    <style>
      ...
      .wrap{padding:18px 18px 8px}.top{display:flex;justify-content:space-between;align-items:center;margin-bottom:14px}.top h1{font-size:28px;margin:0}.top button{width:42px;height:42px;border-radius:50%;background:var(--secondary-background-color)}.playerpick{display:flex;align-items:center;gap:8px;width:100%;padding:10px 12px;margin:0 0 14px;border-radius:14px;background:var(--secondary-background-color);font-weight:700;text-align:left}.playerpick span{flex:1}.picker{display:none;margin:0 0 14px;padding:8px;border-radius:14px;background:var(--secondary-background-color)}.picker.open{display:block}.pickrow{display:flex;align-items:center;width:100%;padding:9px 10px;border-radius:10px;text-align:left}.pickrow.active{background:var(--card-background-color)}.pickrow span{flex:1}.art,.placeholder{aspect-ratio:1/1;width:100%;border-radius:18px;background:var(--secondary-background-color)}
      .art{object-fit:cover;display:block}.placeholder{display:grid;place-items:center;font-size:64px;opacity:.65}.meta{text-align:center}.progress{margin:14px 0 2px}.progress input{width:100%}.times{display:flex;justify-content:space-between;color:var(--secondary-text-color);font-size:11px}.topcopy{text-align:center;min-width:0}.eyebrow{font-size:11px;letter-spacing:.09em;text-transform:uppercase;color:var(--secondary-text-color);font-weight:700}
      ...
      button{appearance:none;border:0;background:none;color:inherit;min-height:48px;cursor:pointer}.activecmd{color:var(--primary-color)}.main{width:68px;height:68px;background:var(--primary-text-color);color:var(--card-background-color);border-radius:50%;justify-self:center}.main ha-icon{--mdc-icon-size:31px}.skip ha-icon{--mdc-icon-size:30px}
    ...
    ${this._view==="now"?`<div class="wrap"><div class="top"><span></span><div class="topcopy"><div class="eyebrow">Now Playing</div><button class="playerpick" id="playerpick"><span>${this._esc(a.friendly_name||"Select Sonos")}</span><ha-icon icon="mdi:chevron-down"></ha-icon></button></div><button class="round"><ha-icon icon="mdi:dots-horizontal"></ha-icon></button></div><div class="picker ${this._pickerOpen?"open":""}" id="playerlist">${this._players.map(p=>`<button class="pickrow ${p.entity_id===this._selected?"active":""}" data-select-player="${p.entity_id}"><ha-icon icon="mdi:speaker"></ha-icon><span>${this._esc(p.attributes?.friendly_name||p.entity_id)}</span>${p.entity_id===this._selected?`<ha-icon icon="mdi:check"></ha-icon>`:""}</button>`).join("")}</div>${art?`<img class="art" src="${art}" alt="">`:`<div class="placeholder">♫</div>`}<div class="meta"><h2>${this._esc(title)}</h2><div class="artist">${this._esc(artist)}</div><div class="album">${this._esc(album)}</div><div class="progress"><input id="seek" type="range" min="0" max="100" value="${progress}" ${duration?"":"disabled"}><div class="times"><span>${fmt(position)}</span><span>${duration?fmt(duration):"--:--"}</span></div></div></div></div>
    ...
    this.shadowRoot.querySelectorAll("[data-select-player]").forEach(el=>el.onclick=()=>{this._selected=el.dataset.selectPlayer;this._pickerOpen=false;this._selectedRooms=new Set(this._hass.states[this._selected]?.attributes?.group_members||[this._selected]);this._render();});
