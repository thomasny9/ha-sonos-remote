class SonosRemoteCard extends HTMLElement {
  ...
  _defaultPlayerSettingsHtml() {
    ...
    return `<div class="fixedsettings"><div class="fixedtitle"><b>Default Sonos player</b><button id="closedefaultplayer"><ha-icon icon="mdi:close"></ha-icon></button></div><small>Choose which Sonos player opens by default on the Now Playing screen.</small>${(this._players||[]).map(p=>`<label class="fixedrow"><span>${this._esc(p.attributes?.friendly_name||p.entity_id)}</span><input type="radio" name="default-sonos-player" data-default-player="${p.entity_id}" ${selected===p.entity_id?"checked":""}></label>`).join("")}<label class="fixedrow"><span>Use card / first available player</span><input type="radio" name="default-sonos-player" data-default-player="" ${!selected?"checked":""}></label></div>`;
  }
  ...
  _render() {
    ...
    this.shadowRoot.querySelectorAll("[data-default-player]").forEach(el=>el.addEventListener("change",async e=>{
      ...
      try{
        ...
        if(entityId&&this._hass.states[entityId]) this._selected=entityId;
    ...
    this.shadowRoot.querySelectorAll("[data-music-service]").forEach(el=>el.addEventListener("change",async e=>{
      e.stopPropagation();
      const id=e.target.dataset.musicService;
      try{
        const r=await this._hass.callWS({type:"sonos_remote/set_music_service_visible",service_id:id,visible:e.target.checked});
        this._backendInfo={...(this._backendInfo||{}),hidden_music_services:r.hidden_music_services||[]};
        if(this._view==="mymusic")this._render();
...
console.info("%c SONOS REMOTE %c v0.5.6 ","color:white;background:#03a9f4;font-weight:bold","color:#03a9f4;background:white");
