class SonosRemoteCard extends HTMLElement {
  ...
  _render() {
    ...
    this._view==="rooms"?`<div class="rooms"><div class="roomhead"><h1>Rooms</h1><button class="round"><ha-icon icon="mdi:dots-horizontal"></ha-icon></button></div>${this._players.map(p=>{const pa=p.attributes||{},v=Math.round((pa.volume_level||0)*100),on=this._selectedRooms.has(p.entity_id);return `<div class="room"><button class="check ${on?"on":""}" data-room="${p.entity_id}">${on?"✓":""}</button><div><div class="roomname">${this._esc(pa.friendly_name||p.entity_id)}</div><div class="roomsub">${this._esc(pa.media_title||"Not Playing")}</div><div class="roomvol"><input data-roomvol="${p.entity_id}" type="range" min="0" max="100" value="${v}"><span>${v}</span></div></div><ha-icon icon="mdi:dots-horizontal"></ha-icon></div>`}).join("")}<button class="apply" id="apply">Apply to ${this._selectedRooms.size} Room${this._selectedRooms.size===1?"":"s"}</button></div>`:this._view==="music"?`<div class="music"><div class="roomhead"><h1>Music</h1><button class="round"><ha-icon icon="mdi:dots-horizontal"></ha-icon></button></div><label class="search"><ha-icon icon="mdi:magnify"></ha-icon><input id="musicsearch" placeholder="Search Sonos music library"></label><div class="sectiontitle">Sonos Favorites</div><div id="favorites">${this._favoritesHtml()}</div><div class="sectiontitle">Browse</div><button class="fav" id="openmedia"><span class="favart"><ha-icon icon="mdi:folder-music"></ha-icon></span><span><span class="favname">Home Assistant Media</span><span class="favsub">Browse media sources available to Home Assistant</span></span><ha-icon icon="mdi:chevron-right"></ha-icon></button></div>`:`<div class="stub"><h2>${this._view[0].toUpperCase()+this._view.slice(1)}</h2><div class="artist">Coming in the next implementation stage</div></div>`}
