class SonosRemoteCard extends HTMLElement {
  ...
  _render() {
    ...
    ${this._view==="now"?`<div class="wrap"><div class="top"><h1>Now Playing</h1><button><ha-icon icon="mdi:dots-horizontal"></ha-icon></button></div><button class="playerpick" id="playerpick"><ha-icon icon="mdi:speaker"></ha-icon><span>${this._esc(a.friendly_name||"Select Sonos")}</span><ha-icon icon="mdi:chevron-down"></ha-icon></button><div class="picker ${this._pickerOpen?"open":""}" id="playerlist">${this._players.map(p=>`<button class="pickrow ${p.entity_id===this._selected?"active":""}" data-select-player="${p.entity_id}"><ha-icon icon="mdi:speaker"></ha-icon><span>${this._esc(p.attributes?.friendly_name||p.entity_id)}</span>${p.entity_id===this._selected?`<ha-icon icon="mdi:check"></ha-icon>`:""}</button>`).join("")}</div>${art?`<img class="art" src="${art}" alt="">`:`<div class="placeholder">♫</div>`}<h2>${this._esc(title)}</h2><div class="artist">${this._esc(artist)}</div><div class="album">${this._esc(album)}</div></div>
