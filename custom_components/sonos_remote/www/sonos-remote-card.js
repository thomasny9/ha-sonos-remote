class SonosRemoteCard extends HTMLElement {
  ...
  _render() {
    ...
    <style>
      :host{display:block} ha-card{overflow:hidden;border-radius:22px;background:#111214;color:#f5f5f5;border:0;box-shadow:0 14px 40px rgba(0,0,0,.28)}
      .wrap{padding:18px 18px 8px}.top{display:flex;justify-content:center;align-items:center;margin-bottom:14px}.playerpick{display:flex;align-items:center;justify-content:center;gap:5px;padding:5px 8px;margin:0;background:transparent;color:#f5f5f5;font-size:20px;font-weight:700}.playerpick span{max-width:280px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.picker{display:none;margin:0 0 14px;padding:8px;border-radius:14px;background:#202124}.picker.open{display:block}.pickrow{display:flex;align-items:center;gap:9px;width:100%;padding:9px 10px;border-radius:10px;text-align:left;color:#f5f5f5}.pickrow.active{background:#303236}.pickrow span{flex:1}.art,.placeholder{aspect-ratio:1/1;width:100%;border-radius:16px;background:#202124}
      .art{object-fit:cover;display:block}.placeholder{display:grid;place-items:center;font-size:64px;opacity:.65}.meta{text-align:center}.progress{margin:14px 0 2px}.progress input{width:100%}.times{display:flex;justify-content:space-between;color:#8f9297;font-size:11px}.topcopy{text-align:center;min-width:0}.eyebrow{font-size:11px;letter-spacing:.09em;text-transform:uppercase;color:#8f9297;font-weight:700}
      ...
      button{appearance:none;border:0;background:none;color:inherit;min-height:48px;cursor:pointer}.activecmd{color:var(--primary-color)}.main{width:68px;height:68px;background:#f5f5f5;color:#111214;border-radius:50%;justify-self:center}.main ha-icon{--mdc-icon-size:31px}.skip ha-icon{--mdc-icon-size:30px}
      ...
      .tabs{display:grid;grid-template-columns:repeat(4,1fr);border-top:1px solid #2a2c2f;background:#151618;padding:7px 4px calc(7px + env(safe-area-inset-bottom))}
    ...
    this.shadowRoot.querySelectorAll("[data-select-player]").forEach(el=>el.onclick=()=>{this._selected=el.dataset.selectPlayer;this._pickerOpen=false;const g=this._groups().find(x=>x.members.includes(this._selected));this._selectedRooms=new Set(g?.members||[this._selected]);this._render();});
