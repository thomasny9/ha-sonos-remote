...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    frontend_path = Path(__file__).parent / "www" / "sonos-remote-card-v058.js"
...
@websocket_api.websocket_command(
    {
        "type": "sonos_remote/set_default_player",
        vol.Required("entity_id"): vol.Any(str, None),
    }
)
@websocket_api.async_response
async def websocket_sonos_remote_set_default_player(hass, connection, msg):
    entity_id = msg["entity_id"]
    if entity_id is not None and entity_id not in _sonos_entities(hass):
        connection.send_error(msg["id"], "invalid_sonos_player", "Entity is not a Sonos media player")
        return
    hass.data.setdefault(DOMAIN, {})["default_player"] = entity_id
    await _save_settings(hass)
    connection.send_result(msg["id"], {"ok": True, "default_player": entity_id})
